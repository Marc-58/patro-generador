...
function dibuixaPatro(p) {
  p.setup = function () {
    const escala = 0.026458333333719;
    const marge = 50;
    ...
    p.line(pinçaX2 + 3 * escala, marge, pinçaX2 + 1.5 * escala, marge + 10 * escala); // línia de pinça dreta
      ...
      p.createCanvas(espatlles + torax + 2 * marge,  talleDavanter + 2 );
      p.background(255);
      p.stroke(0);
      p.noFill();
...
p.line(
 ...
 p.line(10 + cintura / 8 + 1.5 * escala, talleEspatlles  + 10+18*escala, 10 + (cintura / 8), 10 + talleEspatlles); // pinça esquerra
  ...
  const cadera = mides.cadera * escala;
      ...
      p.createCanvas(cadera/2  + 2 * 10*escala, llargVestit+ talleDavanter + 2 * 10);
      p.background(255);
      p.stroke(0);
      p.noFill();
...
p.line(marge + torax / 4 - cintura / 8, 10 + talleDavanter, marge + torax / 4 - cintura / 8 - 1.5 * escala, 10 + talleDavanter+18*escala); // pinça esquerra
 ...
 p.line(10 + cintura / 8 + 1.5 * escala, talleEspatlles  + 10+18*escala, 10 + (cintura / 8), 10 + talleEspatlles); // pinça esquerra
  ...
  // Part de vestit
  p.rect(10, talleEspatlles+18*escala+10, (cadera / 4)+1*escala*0, llargVestit-18*escala);
  p.rect(marge, talleDavanter+18*escala+10, (cadera / 4)-1*escala*0, llargVestit-18*escala);
...
p.curveVertex(marge + contornBraç / 2, marge - 1.5 * escala);
p.curveVertex(marge + contornBraç / 2+1.5*escala, marge - 1 * escala);
...
p.endShape();
    ...
    p.line(marge, marge, marge +radi*2+llarg*2, marge );
}
  };
}
